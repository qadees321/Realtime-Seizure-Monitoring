...
with st.expander("📊 Model analytics", expanded=False):
    ...
    # Derive dataset counts from the actual replay dataset instead of hard-coding
    # the original 11,500 / 2,300 / 9,200 values. This keeps the analytics
    # correct if data/epileptic_seizure_data.csv is replaced with another dataset.
    try:
        analytics_df = load_replay_dataset()
        total = int(len(analytics_df))
        seizure_n = int((pd.to_numeric(analytics_df["y"], errors="coerce") == 1).sum())
        non_n = int(total - seizure_n)
    except Exception:
        # Fall back to metadata when the dataset cannot be loaded.
        class_dist = meta.get("class_distribution", {}) if isinstance(meta, dict) else {}
        seizure_n = int(class_dist.get("seizure", class_dist.get(1, 0)) or 0)
        non_n = int(class_dist.get("non_seizure", class_dist.get("non-seizure", 0)) or 0)
        total = seizure_n + non_n
    ...
    seizure_pct = seizure_n / total if total else 0.0
    non_pct = non_n / total if total else 0.0
    ...
    st.markdown(f'''<div class="analytics-grid">
      ...
      <div class="analytics-stat"><div class="n">{seizure_n:,}</div><div class="l">Seizure windows</div><div class="s">{seizure_pct:.1%} of dataset</div></div>
      <div class="analytics-stat"><div class="n">{non_n:,}</div><div class="l">Non-seizure windows</div><div class="s">{non_pct:.1%} of dataset</div></div>
    ...
    with at1:
        ...
        if not results_df.empty:
            ...
            model_col = next((c for c in display_df.columns if str(c).strip().lower() in {"model","name","model_name","classifier","algorithm"}), None)
            # Some exports store model names in the DataFrame index.
            if model_col is None and not isinstance(display_df.index, pd.RangeIndex):
                display_df = display_df.copy()
                display_df["Model"] = display_df.index.astype(str)
                model_col = "Model"
            # Older metadata exports may have neither a model column nor a named index.
            # Recover the known training-model names by row order so the UI never shows
            # an unnamed comparison table. This matches the project's three-model setup.
            if model_col is None:
                fallback_names = ["Logistic Regression", "Random Forest", "SVM"]
                if len(display_df) <= len(fallback_names):
                    display_df = display_df.copy()
                    display_df.insert(0, "Model", fallback_names[:len(display_df)])
                    model_col = "Model"
                elif isinstance(meta, dict):
                    meta_names = meta.get("model_names") or meta.get("models") or meta.get("candidate_models")
                    if isinstance(meta_names, (list, tuple)) and len(meta_names) == len(display_df):
                        display_df = display_df.copy()
                        display_df.insert(0, "Model", [str(x) for x in meta_names])
                        model_col = "Model"
    ...
    with at2:
        st.markdown(f'<div class="analytics-note"><b>Diagnostics model:</b> {trained_model_name} · deployed estimator: {model.__class__.__name__} · threshold: {threshold:.1%}</div>', unsafe_allow_html=True)
        d1,d2=st.columns(2)
        with d1:
        ...
        if not validation_df.empty:
            ...
            vmodel_col = next((c for c in validation_display.columns if str(c).strip().lower() in {"model","name","model_name","classifier","algorithm"}), None)
            if vmodel_col is None and not isinstance(validation_display.index, pd.RangeIndex):
                validation_display["Model"] = validation_display.index.astype(str)
                vmodel_col = "Model"
            if vmodel_col is None:
                fallback_names = ["Logistic Regression", "Random Forest", "SVM"]
                if len(validation_display) <= len(fallback_names):
                    validation_display.insert(0, "Model", fallback_names[:len(validation_display)])
